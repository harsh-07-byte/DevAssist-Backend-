...
exports.createRating = async (req,res)=>{
  try {
    ...
    const courseDetails= await Course.find({_id: courseId,
     ...
     if(!courseDetails){
         return res.status(404).json({success:false,emessage: "No such student found in this course"});
     };
     const alreadyReviewed =await RatingAndReview.findOne({user:userId,
     course:courseId});
 
     if(alreadyReviewed){
         return res.status(404).json({success: false,message: "This student has already reviwed"});
     }
